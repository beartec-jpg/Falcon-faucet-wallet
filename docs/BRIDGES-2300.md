...
**ETH/USDC dest-lock in and out are live.** Wallet deposit auto-mints on Falcon PL. Bridge out burns on Falcon PL, waits for a sampled LC header, then `openClaim`/`take` dest-locked to your Sepolia 0x.

**BTC dest-lock in and out are live** (`BTC_RAIL_LIVE = true`). Peg-in: testnet BTC to the FROST P2TR vault + FALC dest memo, then SPV mint after 6 confirmations. Peg-out: burn FBTC, operator FROST Kickoff dest-lock, CSV=6 take to your BTC address. Do not send to old 1001 collateral locks.
...
BTC vault (FROST P2TR, t=4 of n=6): `tb1pj9d6d6eaayw7f7hc3mr2lm3xhuscuhtw6kpjqz5jvvuf4mh2lduq2pqytm`
