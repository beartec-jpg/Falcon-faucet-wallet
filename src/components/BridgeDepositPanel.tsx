...
export default function BridgeDepositPanel({
  ...
  const [fbtcIssuer, setFbtcIssuer] = useState('')
  ...
  useEffect(() => {
    ...
    let cancelled = false
    void (async () => {
      const cfg = await fetchPl2300BridgeConfig()
      if (cancelled) return
      setDestLockCfg(cfg)
      if (!cfg) return
      try {
        const tip = await fetchFplTip(cfg)
        if (!cancelled) setDestLockTip(tip)
      } catch {
        if (!cancelled) setDestLockTip(null)
      }
    })()
    return () => {
      cancelled = true
    ...
    if (isPl2300) {
      ...
      for (let attempt = 1; attempt <= 5; attempt++) {
        try {
          ...
          if (/tecDUPLICATE|already spent|already minted/i.test(m)) throw e
          if (/wrong watch|retired watch|not a live falcon btc watch/i.test(m)) throw e
          if (!isSpvWaitMessage(m) || attempt === 5) throw e
          lastWait = e
          setStep(
      ...
      if (!minted) throw lastWait instanceof Error ? lastWait : new Error('Claim failed')
