...
export async function POST(req: NextRequest) {
  ...
  if (action === 'eth-kickoff') {
    ...
    if (!/^0x[a-fA-F0-9]{64}$/.test(noteId) && !/^[a-fA-F0-9]{64}$/.test(noteId)) {
      return NextResponse.json({ error: 'noteId must be 32-byte hex' }, { status: 400 })
    }
  ...
  if (action === 'btc-kickoff' || action === 'btc-take') {
    ...
    if (!/^[0-9]+$/.test(amount) || Number(amount) < 2000) {
      return NextResponse.json({ error: 'amount must be ≥ 2000 sats' }, { status: 400 })
    }
    if (action === 'btc-take' && !/^[0-9a-f]{64}$/.test(destSecret)) {
