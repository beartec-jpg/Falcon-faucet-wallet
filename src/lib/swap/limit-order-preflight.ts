...
/** Price is FPL per F-USDC (same as order form). */
...
export function limitOrderWouldCross(
  ...
  if (side === 'sell') {
    // Cheaper ask (lower FPL/F-USDC) crosses AMM and bids.
    ...
    // Richer bid (higher FPL/F-USDC) crosses AMM and asks.
    if (marketPrice != null && marketPrice > 0 && price >= marketPrice * (1 - 1e-9)) return true
...
export function restingPriceHint(
  ...
  return side === 'sell'
    ? `List a sell on the book: enter more than ~${marketPrice.toFixed(2)} FPL per F-USDC (try ${maker}). Lower numbers sell cheaper and hit the AMM.`
    : `List a buy on the book: enter less than ~${marketPrice.toFixed(2)} FPL per F-USDC (try ${maker}). Higher numbers pay more and hit the AMM.`
}
...
export function limitOrderPreflightReason(opts: {
  ...
  if (!Number.isFinite(price) || price <= 0) return 'Enter a valid price (FPL per F-USDC).'
  ...
  if (crosses && mid != null) {
    if (side === 'sell') {
      return `Your price ${price} is below AMM mid ~${mid.toFixed(2)} FPL per F-USDC — this sells F-USDC too cheap and will execute immediately through the AMM, not rest on the book. Post only does not stop AMM fills. To list, use at least ~${maker ?? mid.toFixed(2)} (higher number = higher ask).`
    }
    return `Your price ${price} is above AMM mid ~${mid.toFixed(2)} FPL per F-USDC — this bid pays too much and will execute immediately through the AMM. To list, use below ~${maker ?? mid.toFixed(2)} (lower number = lower bid).`
  }
...
export function explainOfferResult(
  ...
  if (engineResult === 'tecUNFUNDED_OFFER') {
    return 'Insufficient balance to fund this offer (check F-USDC or FPL).'
  }
  return null
}